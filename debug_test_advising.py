...
schema_linker.table_linking(db_name="advising", question=question)
print("****************Table linking result****************")
print(schema_linker.table_linking_result)

schema_linker.column_linking(db_name="advising", question=question)
...
db_samples = schema_linker.get_db_samples("advising")
for sample in db_samples:
    print(sample)
print("****************FilteredDB Description****************")
print(schema_linker.get_db_description(db_name="advising"))
...
sg.retrieve_examples(db_name="advising", question=question)

sg.generate_sql(question=question, schema_linker=schema_linker)
sg.candidate_filtering("advising")
sg.multiple_choices_selection(db_name = "advising", question=question, schema_linker=schema_linker)
print("****************Result SQL****************")
print(sg.final_sql)
